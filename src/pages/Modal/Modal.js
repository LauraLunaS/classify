...
const Modal = ({ isOpen, onRequestClose }) => {
    ...
    const fetchCursos = async () => {
      try {
        ...
        if (dataCursos) {
          ...
          console.error('Results not found or not in the expected format in API response:', dataCursos);
        }
    ...
    const fetchDisciplinas = async () => {
      try {
        ...
        if (!responseDisciplinas.ok) {
          throw new Error(`Failed to fetch disciplinas: ${responseDisciplinas.status}`);
        }
        ...
        if (dataDisciplinas) {
          ...
          console.error('Results not found or not in the expected format in API response:', dataDisciplinas);
        }
    ...
    const fetchProfessores = async () => {
      try {
        ...
        if (!responseProfessores.ok) {
          throw new Error(`Failed to fetch professores: ${responseProfessores.status}`);
        }
        ...
        if (dataProfessores) {
          ...
          console.error('Results not found or not in the expected format in API response:', dataProfessores);
        }
    ...
    const fetchSalas = async () => {
      try {
        ...
        if (dataSalas) {
          ...
          console.error('Results not found or not in the expected format in API response:', dataSalas);
        }
    ...
    const fetchPeriodos = async () => {
      try {
        ...
        if (!responsePeriodos.ok) {
          throw new Error(`Failed to fetch periodos: ${responsePeriodos.status}`);
        }
        ...
        if (dataPeriodos) {
          ...
          console.error('Results not found or not in the expected format in API response:', dataPeriodos);
        }
  ...
  const buscarSalasDisponiveis = () => {
    ...
    const salasDisponiveis = salas.filter((sala) => sala.capacity >= quantidadeAlunosInt);
  ...
  return (
    <div className={style.Container}>
      ...
      <div className={style.Content}>
        ...
        <select className={style.inputMenor} onChange={e => setPeriodoSelecionado(e.target.value)}>
